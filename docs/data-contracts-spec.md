...
| `full_refresh_method` | string | No | Full refresh: clear method override -- `auto` (default), `delete`, or `truncate`. See the full-refresh clear-method note below. |
...
**Full-refresh clear method (`full_refresh_method`):**
...
Overrides how a `full_refresh` write clears the target before reloading:

- `auto` (default): TRUNCATE at or above the resource `full_refresh_threshold`,
  DELETE below it. The single-DataFrame path sizes this on the incoming frame;
  the streamed (batched) path has no incoming count and sizes it on the
  target's `pg_class.reltuples`, falling back to DELETE when the target has
  never been analyzed (mirror issue model-oncology-public/moncpipelib#4).
- `delete`: always DELETE. Lower lock (ROW EXCLUSIVE), allows concurrent reads.
  This is the escape hatch for a target TRUNCATE cannot clear -- notably one
  **referenced by a foreign key** from another table, where TRUNCATE raises
  `cannot truncate a table referenced in a foreign key constraint` (and
  `ON DELETE CASCADE` does not apply to TRUNCATE).
- `truncate`: always TRUNCATE. Faster (O(1)), but takes ACCESS EXCLUSIVE and
  does not fire `ON DELETE` triggers.

The caller-supplied side of the four-way reconciliation for this field is the
resource `write(..., full_refresh_method=...)` keyword argument. Resolution
order is: `write()` kwarg / contract sink (reconciled) over the resource-level
`full_refresh_method` default. The retired IO-manager path does not accept a
`full_refresh_method` asset-metadata key; contract sinks work on both paths.
...
`skip_unchanged`, `full_refresh_method`) follow the same four-way rule:
...
- `skip_unchanged: true` on a sink whose declared `mode` is not `upsert` is
  ...
  merge).
- `full_refresh_method` must be one of `auto` / `delete` / `truncate`, and is
  rejected on a sink whose declared `mode` is not `full_refresh` (the field is
  consumed only by the full-refresh clear step).
- These mode-scoped fields are also backstopped at write time:
  `validate_write_config` rejects `detect_deletes=True` outside `scd2`,
  `skip_unchanged=True` outside `upsert`, and a `full_refresh_method` set
  outside `full_refresh`, covering configuration whose mode arrives via asset
  metadata or the `write()` call rather than a declared sink mode.
...
DataFrames carrying NULLs in any `primary_key` column at write time. Likewise,
a declared `partition_column` must be present in the outgoing DataFrame and
free of NULLs at write time, because `NULL IN (...)` never matches, leaving
such rows unreachable by every partition-scoped clear (#480).
