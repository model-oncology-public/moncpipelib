...
class BatchedDataFrame:
    ...
    Attributes:
        ...
        total_rows_hint: Optional estimated total rows. If provided, log
            messages include percentage progress -- but that is no longer
            its only role. It is also forwarded down the write path to size
            the COPY-vs-parameterized-INSERT decision (#456) and the
            full_refresh clear-method decision (model-oncology-public/
            moncpipelib#4) for batches that are individually too small to
            reach either threshold on their own. Still an estimate --
            it does not need to be exact -- but an inexact value now changes
            which *mechanism* a batch takes (COPY vs execute_values,
            TRUNCATE vs DELETE) and therefore how many statements / audit-log
            entries the write produces, not just a progress percentage. Since
            the COPY and parameterized insert encodings are
            fidelity-equivalent (see
            ``io_managers.writers.serialize_for_staging_copy``), an inexact
            hint never changes the row values actually written -- only
            mechanism and audit shape.
...
def transform_batched(
    ...
    *,
    ...
    Args:
        ...
        total_rows_hint: Optional estimated total rows. Passed through to
            the BatchedDataFrame -- besides progress logging, the write path
            uses it to size the COPY-vs-parameterized-INSERT and
            full_refresh clear-method decisions (#456, model-oncology-public/
            moncpipelib#4) when per-batch counts alone would never reach
            either threshold. See ``BatchedDataFrame.total_rows_hint``.
