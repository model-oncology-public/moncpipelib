...
class PostgresResource(ConfigurableResource):
    ...
    """Rows packed into each multi-row INSERT statement on the executemany
    (``bulk_insert_method="execute_values"``) path. Defaults to
    :data:`~moncpipelib.config.EXECUTEMANY_PAGE_ROWS_DEFAULT` (200).

    Each page is one statement execution, and therefore one pgaudit
    session-audit line -- at the pre-#456 default of one row per execution, a
    392k-row streamed load produced roughly 731k pgaudit lines. Raising this
    divides the audited-statement count by (approximately) the page size,
    with no change to what is written.

    The effective page size is capped at runtime to
    ``PG_MAX_BIND_PARAMS // n_cols`` (PostgreSQL's extended-query protocol
    caps bind parameters at 65,535 per statement), so a very wide table
    automatically uses a smaller page than requested rather than failing.
    Row values are always passed as bind parameters, never inlined as SQL
    literals, at any page size -- the audited statement text never contains
    row data.

    Must be ``>= 1``; ``_get_writer_config`` raises ``ValueError`` for values
    below that. Unlike ``insert_chunk_size``, ``0`` does NOT mean "disabled"
    here -- ``_executemany_bulk`` has no unpaged mode, and one row per
    execution (``page_rows=1``) is exactly the pgaudit-volume problem #456
    fixed, so it is never treated as an implicit "off" state.
    """

    full_refresh_method: str = "auto"
    """Method for clearing tables in full_refresh mode. Resource-level default;
    a contract sink or the ``write(full_refresh_method=...)`` kwarg overrides it
    per-write.
    - auto: TRUNCATE at or above the threshold, DELETE below it (default)
    - delete: Always use DELETE (safer locking, allows concurrent reads; the
      escape hatch for a target TRUNCATE cannot clear, e.g. one referenced by a
      foreign key from another table -- #4)
    - truncate: Always use TRUNCATE (faster, but holds ACCESS EXCLUSIVE)
    """
    ...
    Sized on the incoming DataFrame for single-frame writes. Streamed (batched)
    writes carry no such count -- ``total_rows_hint`` is optional progress
    metadata -- so they size it on the target's existing ``pg_class.reltuples``,
    which is the quantity the clear actually has to remove. A target that has
    never been analyzed yields no estimate and stays on DELETE (#4)."""
    ...
    *target-reading* anti-join statements (count, expire, Stage-1 CTAS, and
    ...
    def write(
        ...
        *,
        ...
        full_refresh_method: str | None = None,
    ) -> WriteResult:
        ...
        Args:
            ...
            analyze_after_write: Post-commit ``ANALYZE`` behavior for this
                ...
                semantics.
            full_refresh_method: Per-write override of the full_refresh clear
                method: ``"auto"`` / ``"delete"`` / ``"truncate"``. ``None``
                (default) = use the contract sink's ``full_refresh_method`` if
                set, else the resource-level ``full_refresh_method``. Pin
                ``"delete"`` for a target that TRUNCATE cannot clear (e.g. one
                referenced by a foreign key from another table). Only meaningful
                for ``full_refresh`` writes.
    ...
    def _get_writer_config(self, full_refresh_method: str | None = None) -> WriterConfig:
        """Build a ``WriterConfig`` from this resource's attributes.

        ``full_refresh_method`` is an optional per-write override (from a
        contract sink or the ``write()`` kwarg, reconciled into
        ``write_config``); when ``None`` the resource-level default applies.
        Enum construction validates the string in either case.

        Raises:
            ValueError: If ``executemany_page_rows < 1``. Unlike
                ``insert_chunk_size``, ``0`` is not a valid "disabled" value
                here -- ``_executemany_bulk`` has no unpaged mode, and
                per-row emission (``page_rows=1``) is exactly what #456
                fixed, so a non-positive value is always a misconfiguration
                rather than an intentional opt-out. Failing fast here keeps
                the bad value from reaching ``_executemany_bulk``'s runtime
                ``max(1, ...)`` clamp, which would silently coerce it
        ...
        if self.executemany_page_rows < 1:
            raise ValueError(
                f"executemany_page_rows must be >= 1, got {self.executemany_page_rows}. "
                f"Unlike insert_chunk_size, 0 does not mean 'disabled' -- "
                f"_executemany_bulk has no unpaged mode, and per-row emission "
                f"(page_rows=1) is exactly the pgaudit-volume problem #456 fixed."
            )

        effective_method = (
            self.full_refresh_method if full_refresh_method is None else full_refresh_method
        )
        return _WriterConfig(
            ...
            full_refresh_method=FullRefreshMethod(effective_method),
            full_refresh_threshold=self.full_refresh_threshold,
            insert_chunk_size=self.insert_chunk_size,
            executemany_page_rows=self.executemany_page_rows,
        )
    ...
    def _build_write_config(
        *,
        ...
        return {
            ...
            "analyze_after_write": analyze_after_write,
            "full_refresh_method": full_refresh_method,
            "full_refresh_method_explicit": full_refresh_method is not None,
            # SCD2 defaults
            "scd2": SCD2Config(),
    ...
    def _validate_partition_column_values(
        self,
        df: pl.DataFrame,
        partition_column: str | None,
        asset_name: str,
        ...
        """Validate a declared partition_column is present and NULL-free (thin wrapper).

        Delegates to :func:`_contract_helpers.validate_partition_column_values`.

        Raises:
            ContractViolationError: If ``partition_column`` is declared but
                missing from ``df``, or present with one or more NULL values.
        """
        from moncpipelib.resources._contract_helpers import validate_partition_column_values

        validate_partition_column_values(df, partition_column, asset_name, table_name)
    ...
    def _write_single(
        ...
        *,
        ...
        # #480: a declared partition_column must be present and NULL-free --
        # NULL never matches the partition-scoped DELETE below, so a NULL
        # row would duplicate on every re-land. Runs BEFORE
        # ``_validate_write_config`` (mirroring the batched path's
        # ordering) so a missing partition_column raises this guard's
        # detailed ``ContractViolationError`` rather than being shadowed by
        # ``_validate_write_config``'s bare ``ValueError`` for the same
        # defect. The guard depends on nothing ``_validate_write_config``
        # produces -- it reads only ``df`` and
        # ``write_config["partition_column"]``.
        self._validate_partition_column_values(
            df, write_config["partition_column"], wctx.asset_name, table_name
        )
        ...
        try:
            with conn.cursor() as cursor:
                ...
                config = self._get_writer_config(
                    full_refresh_method=write_config.get("full_refresh_method")
                )
                if write_mode == WriteMode.FULL_REFRESH:
    ...
    def _write_batched(
        ...
        *,
        ...
        # Full-refresh clear outcome, surfaced in final_stats and fed to the
        # post-write ANALYZE gate. ``types.py`` documents ``clear_method`` as
        # part of the returned stats; before #4 the batched path discarded it,
        # which is why the AUTO regression it fixes was invisible in Dagster
        # metadata.
        ...
        try:
            with conn.cursor() as cursor:
                ...
                for i, batch_df in enumerate(batched.batches):
                    ...
                    # #480: a declared partition_column must be present and
                    # NULL-free on EVERY batch (not just i == 0) -- a NULL
                    # can appear in any batch, and never matches the
                    # partition-scoped DELETE, so it would duplicate on
                    # every re-land.
                    self._validate_partition_column_values(
                        batch_df, write_config["partition_column"], wctx.asset_name, table_name
                    )
                    ...
                    if i == 0:
                        ...
                        if write_mode == WriteMode.FULL_REFRESH:
                            # Build (and thereby validate) the writer config up
                            # front, mirroring the single-DataFrame path so an
                            # invalid full_refresh_method override fail-fasts on
                            # both the partition-scoped and full-table branches
                            # -- even though the partition-scoped clear always
                            # DELETEs by partition regardless of the method (#4).
                            config = self._get_writer_config(
                                full_refresh_method=write_config.get("full_refresh_method")
                            )
                            if partition_values is not None and partition_column is not None:
                                ...
                                # Record the deletion so the change gate sees an
                                # emptied partition even when this run loads 0
                                # rows -- the single-DataFrame partition path
                                # signals this via its rows_deleted counter.
                                # Deliberately do NOT set clear_method: that path
                                # (writers.execute_partition_scoped) reports none,
                                # so emitting "delete_partition" only on the
                                # batched branch would give the same logical
                                # operation a path-dependent stats shape.
                    ...
                    if write_mode == WriteMode.SCD2:
                        ...
                        config = self._get_writer_config()
                        # Same total_rows_hint pattern as the ``clear_table``
                        # call above (public mirror issue #4): the batched
                        # path's only size signal for the whole stream is
                        # ``total_rows_hint``, since each batch here only
                        # knows its own row count. Forwarding it lets AUTO
                        # size the COPY decision on the stream total rather
                        # than every small batch individually (#456).
                        insert_rows(
                            config,
                            cursor,
                            table_name,
                            ...
                            write_mode,
                            wctx,
                            total_rows_hint=batched.total_rows_hint,
                        )
                ...
                # autoanalyzed by autovacuum, so refresh their aggregate
                # stats here (mirror issue model-oncology-public/moncpipelib#1).
                # Feed the full-refresh clear outcome into the change gate so a
                # TRUNCATE that emptied a populated target still analyzes even
                # when it then loaded nothing (#4); SCD2 carries its own
                # counters, and the plain batched insert path still falls back
                # to total_rows.
                if scd2_stats is not None:
                    analyze_stats: dict[str, Any] = dict(scd2_stats)
                else:
                    # A clear ran (full-table TRUNCATE/DELETE, or a partition
                    # DELETE) iff rows_deleted was set. Surface its counters --
                    # and clear_method when the full-table path set one -- so the
                    # change gate sees an emptied target even when this run loaded
                    # 0 rows. The plain batched insert path leaves both None and
                    # falls back to total_rows.
                    analyze_stats = {}
                    if clear_method is not None:
                        analyze_stats["clear_method"] = clear_method
                    if rows_deleted is not None:
                        analyze_stats["rows_deleted"] = rows_deleted
                        analyze_stats["rows_inserted"] = total_rows
                analyze_action = self._analyze_after_write(
                    conn,
                    schema=schema,
                    bare_table=bare_table,
        ...
        # (``writers.execute_full_refresh`` / ``execute_partition_scoped``) so
        # ``types.py``'s documented "rows_deleted, rows_inserted, clear_method"
        # holds on both paths. Before #4 the batched path discarded these, which
        # is why the AUTO clear-method regression was invisible in Dagster
        # metadata. ``clear_method`` is emitted only by the full-table clear
        # (the partition-scoped branch leaves it None to match
        # ``execute_partition_scoped``), but ``rows_deleted``/``rows_inserted``
        # are surfaced whenever a clear ran, gated on ``rows_deleted`` so the
        # partition-scoped branch reports them too.
        if clear_method is not None:
