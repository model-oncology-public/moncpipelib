...
- :func:`validate_partition_column_values`
...
def validate_write_config(
    ...
    # Same inert-config rationale for full_refresh_method (#4): it is consumed
    # only by the full-refresh clear step, and the loader's static check cannot
    # fire when the mode arrives via asset metadata / write() kwarg instead of a
    # declared sink mode. A value set here is non-None only when explicitly
    # provided (contract sink or write() kwarg), so it signals intent that a
    # non-full_refresh write would silently discard.
    if write_config.get("full_refresh_method") is not None and write_mode != WriteMode.FULL_REFRESH:
        raise ValueError(
            f"full_refresh_method on asset '{asset_name}' is only valid with "
            f"write_mode='full_refresh', got '{write_mode.value}'."
        )
...
def validate_partition_column_values(
    df: pl.DataFrame,
    partition_column: str | None,
    asset_name: str,
    table_name: str,
) -> None:
    """Validate that a declared partition_column is present and NULL-free.

    Every partition-scoped clear issues ``DELETE FROM t WHERE
    "<partition_column>" IN (...)``.  SQL ``NULL IN (...)`` evaluates to
    UNKNOWN, never TRUE, so a row whose partition column is NULL (or
    entirely absent, which lands as NULL) is permanently unreachable by
    that clear: it survives every re-land, duplicates on each one, and is
    invisible to every partition-scoped guard while all of them report
    success (#480).

    The only passing states are: no ``partition_column`` is declared, or
    the column is present in ``df`` with zero NULLs. Every other shape --
    including an absent column and a 0-row DataFrame missing the column --
    raises.
    ...
    if partition_column not in df.columns:
        raise ContractViolationError(
            f"Asset '{asset_name}' declares partition_column '{partition_column}' "
            f"for table '{table_name}', but the outgoing DataFrame does not "
            f"contain that column, so the partition value was not resolved. "
            f"Possible causes: no data_source period whose effective_from "
            f"matches this write supplied a partition_key (not applicable to "
            f"from_ingest sources, whose partition keys resolve through the "
            f"ingest boundary); no Dagster partition key was available on "
            f"the write context; or the asset builds the column itself and "
            f"did not. The write cannot be partition-scoped safely without "
            f"this column. Populate '{partition_column}' before writing, or "
            f"remove partition_column if the write should not be "
            f"partition-scoped.",
            asset_name=asset_name,
        )

    null_count = df[partition_column].null_count()
    if null_count > 0:
        total = len(df)
        raise ContractViolationError(
            f"Asset '{asset_name}' writing to '{table_name}': partition_column "
            f"'{partition_column}' contains NULLs ({null_count} of {total} "
            f'row(s)). NULL never matches DELETE ... WHERE "{partition_column}" '
            f"IN (...), so NULL-partition rows are unreachable by every "
            f"partition-scoped clear and duplicate on every re-land. Ensure "
            f"the partition value is populated for every row.",
            asset_name=asset_name,
        )
