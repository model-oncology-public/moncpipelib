...
from moncpipelib.ingest._cadence import WEEKDAY_BY_NAME, cadence_boundary
...
class CalendarReleaseResolver:
    ...
    def validate_config(self, config: dict[str, Any]) -> list[str]:
        ...
        if cadence == "weekly":
            if not anchor_dow_present:
                ...
                elif value not in WEEKDAY_BY_NAME:
                    known = sorted(WEEKDAY_BY_NAME)
    ...
    def current_release(
        ...
        boundary = cadence_boundary(
            ...
            anchor_tz=str(config.get("anchor_tz", "UTC")),
            anchor_dow=(str(config["anchor_dow"]) if "anchor_dow" in config else None),
        )
