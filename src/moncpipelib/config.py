...
EXECUTEMANY_PAGE_ROWS_DEFAULT: int = 200
"""Default rows packed into each multi-row INSERT statement on the executemany
bulk-insert path (#456). Shared canonical default for
``WriterConfig.executemany_page_rows`` and ``PostgresResource.executemany_page_rows``
-- single-sourced here so the two never drift."""
