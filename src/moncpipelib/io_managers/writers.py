...
# PostgreSQL's extended-query protocol encodes the bind-parameter count as an
# Int16 in the Bind message, so a single statement execution cannot carry more
# than 65,535 parameters -- exceeding it raises server-side regardless of
# what the driver does.  ``_executemany_bulk`` uses this as the ceiling when
# sizing how many rows it packs into one page.
...
def _executemany_bulk(
    ...
    *,
    ...
    """Execute a bulk INSERT via psycopg3's pipelined ``executemany``, paged.
    ...
    ``executemany`` pipelines one INSERT statement execution per call in the
    ``params_seq`` it is given, over the extended-query protocol, which
    requires per-column placeholders.

    Paging rationale (#456): each ``executemany``/``execute`` *execution* is
    one audited statement -- with pgaudit's session log enabled, one
    ``WRITE`` line per execution.  At ``page_rows=1`` (the original shape)
    that is one line per row: a 392k-row streamed load produced roughly 731k
    pgaudit lines, a volume that became an operational and storage problem in
    its own right, independent of the load's cost. Packing ``page_rows`` rows
    into each execution's placeholder list divides the audited-statement
    count by the page size, with no change to what is written.

    Two statement shapes are issued, both built from parameterized
    placeholders only -- row values are never formatted into the SQL text:

    - Zero or more *full pages*, each a single ``VALUES (...), (...), ...``
      statement with exactly ``page_rows`` row-tuples, executed once via one
      ``cursor.executemany`` call carrying one flattened parameter list per
      page (this batches every full page into one pipelined round-trip
      rather than one ``executemany`` call per page).
    - At most one *tail* statement for ``len(rows) % page_rows`` leftover
      rows (skipped entirely when there is no remainder), issued as a single
      ``cursor.execute`` with its own row-count-sized placeholder list.

    ``page_rows`` is capped so no single statement can exceed
    :data:`PG_MAX_BIND_PARAMS` bind parameters: ``effective_page_rows =
    max(1, min(page_rows, PG_MAX_BIND_PARAMS // n_cols))``.  A table wide
    enough that even one row would exceed the ceiling (``n_cols >
    PG_MAX_BIND_PARAMS``) falls back to ``effective_page_rows = 1`` --
    today's per-row behavior -- and lets the driver raise, exactly as it
    would have before this change.

    Compliance guarantee: at every page size, row values travel exclusively
    as bind parameters (``cursor.executemany``/``cursor.execute``'s second
    argument), never interpolated into the SQL string.  With
    ``pgaudit.log_parameter`` off (the default operational posture), the
    audited statement text shows the parameterized placeholders and the
    values themselves are not present in the log at all -- unlike
    psycopg2's ``execute_values``, which mogrified literal values directly
    into the statement text it sent to the server.  This holds for
    ``page_rows=1`` and every larger page size alike; increasing the page
    size only reduces how many audited statements a load produces, not
    what appears in any one of them.

    Args:
        cursor: Database cursor.
        sql: INSERT template containing a single ``VALUES %s`` placeholder.
        rows: Row tuples/sequences to insert, in order.
        page_rows: Target number of rows per statement execution. Required
            (keyword-only, no default) -- both production call sites
            (:func:`insert_with_execute_values`) already pass
            ``config.executemany_page_rows`` explicitly, and a silent
            default here would let a future call site accidentally revert
            to the pre-#456 per-row shape without anyone noticing. Capped
            at runtime by the per-column bind-parameter ceiling; see above.

    Raises:
        ValueError: If ``rows`` contains only zero-column tuples (a
            malformed row shape -- there is no ``VALUES %s`` placeholder to
            build for it).
    """
    if not rows:
        return
    ...
    if n_cols == 0:
        raise ValueError(
            "_executemany_bulk received row(s) with zero columns; cannot "
            "build a VALUES placeholder for an empty row shape."
        )
    effective = max(1, min(page_rows, PG_MAX_BIND_PARAMS // n_cols))

    row_placeholder = "(" + ", ".join(["%s"] * n_cols) + ")"

    def _sql_for(n_tuples: int) -> str:
        """Rewrite the single ``VALUES %s`` placeholder for ``n_tuples`` rows."""
        return sql.replace("VALUES %s", "VALUES " + ", ".join([row_placeholder] * n_tuples), 1)

    if effective == 1:
        # Preserve the original behavior exactly: one row-tuple placeholder,
        # one pipelined executemany call over the untouched row sequence.
        cursor.executemany(_sql_for(1), rows)
        return

    n_full_pages = len(rows) // effective
    remainder = len(rows) % effective

    if n_full_pages > 0:
        page_sql = _sql_for(effective)

        def _page_params() -> Iterator[list[Any]]:
            """Yield one flattened page at a time -- never a full list of pages.

            psycopg3's ``executemany`` consumes ``params_seq`` lazily,
            pipelining one page per iteration, so materializing every page
            up front (the pre-fix ``page_params = [...]`` list
            comprehension) held a second full copy of the paged rows in
            memory for no benefit. Each page is flattened by indexing
            ``rows[base + r][c]`` directly rather than slicing ``rows`` into
            a page-sized list of row-tuples first, so no intermediate
            per-page list is built either -- one flattened list per page,
            nothing more.
            """
            for page_idx in range(n_full_pages):
                base = page_idx * effective
                yield [rows[base + r][c] for r in range(effective) for c in range(n_cols)]

        cursor.executemany(page_sql, _page_params())

    if remainder:
        tail_rows = rows[n_full_pages * effective :]
        tail_sql = _sql_for(remainder)
        tail_params = [v for row in tail_rows for v in row]
        cursor.execute(tail_sql, tail_params)
...
class WriterConfig:
    ...
    executemany_page_rows: int = EXECUTEMANY_PAGE_ROWS_DEFAULT
...
def should_use_copy(
    ...
    *,
    ...
    Args:
        config: Writer configuration.
        row_count: Number of rows in the incoming DataFrame (or batch, for a
            streamed batched write).
        write_mode: The write mode being used.
        total_rows_hint: Optional estimated total rows across the whole
            stream, for a batched write whose individual batches are each
            insert via a separate call to this function.  A large streamed
            load broken into small per-batch parts (e.g. ~1,300 rows each)
            never reaches ``bulk_insert_threshold`` on any single batch, so
            without this hint AUTO would size every batch independently and
            never select COPY no matter how large the stream total is
            (issue #456). Passing the stream's estimated total lets AUTO size
            on ``max(row_count, total_rows_hint)`` instead.  ``None`` or ``0``
            means "no estimate available" and contributes nothing to the
            max -- the decision then falls back to ``row_count`` alone,
            identical to omitting the hint entirely.  The hint only ever
            pushes the decision *toward* COPY, never away from it: a batch
            already at or above the threshold on its own is never downgraded
            by a small or absent hint.

    Returns:
        True if COPY protocol should be used, False for execute_values.

    Note:
        This predicate is pure and never sees the DataFrame -- it cannot
        tell whether COPY can actually serialize it. Callers additionally
        gate on frame COPY-compatibility (see :func:`_frame_supports_copy`);
        prefer :func:`_decide_use_copy`, which wraps both checks together.
    """
    # COPY doesn't support ON CONFLICT, so exclude upsert and scd2. This gate
    # (and an explicit bulk_insert_method) take precedence over the hint --
    # neither branch below considers it.
    ...
    if config.bulk_insert_method == BulkInsertMethod.COPY:
        ...
        return False
    else:  # AUTO - decide based on DataFrame size, sized up by the stream hint
        return max(row_count, total_rows_hint or 0) >= config.bulk_insert_threshold


def _frame_supports_copy(df: pl.DataFrame) -> bool:
    """Whether ``df`` can be CSV-serialized for a COPY-based insert.

    ``df.write_csv`` raises ``ComputeError: CSV format does not support
    nested data`` for any column whose dtype is nested -- ``pl.List``,
    ``pl.Array``, or ``pl.Struct`` (confirmed on the pinned polars version
    via ``dtype.is_nested()``, which covers all three). A ``pl.List(pl.Utf8)``
    column bound for a Postgres ``text[]`` target works fine on the
    parameterized executemany path -- psycopg adapts a Python ``list`` to a
    Postgres array natively -- so the crash is specific to the CSV route,
    not to nested data in general.

    Returns:
        ``False`` if any column's dtype is nested; ``True`` otherwise.
    """
    return not any(dtype.is_nested() for dtype in df.schema.dtypes())


def _decide_use_copy(
    config: WriterConfig,
    df: pl.DataFrame,
    row_count: int,
    write_mode: WriteMode,
    ...
    *,
    ...
    """Resolve the COPY-vs-parameterized decision, gated on frame compatibility.

    ``should_use_copy`` alone doesn't see ``df`` and can select COPY for a
    schema COPY cannot serialize (nested dtypes -- see
    :func:`_frame_supports_copy`). This wraps it with that additional gate,
    used at every ``use_copy`` decision site (:func:`insert_rows`,
    :func:`execute_full_refresh`, :func:`execute_append`,
    :func:`execute_partition_scoped`) so none of them can crash on a nested
    column, whether COPY was selected by AUTO sizing or by an explicit
    ``bulk_insert_method="copy"``.

    When an explicit ``bulk_insert_method="copy"`` is what triggered the
    now-overridden decision, warns loudly -- the operator asked for COPY
    specifically and is silently getting a different (still correct, just
    slower) path instead. AUTO sizing that happens to land on COPY is not
    warned: it was never a hard requirement, just a sizing heuristic, and
    the fallback is exactly as correct.

    Returns:
        Whether to actually use COPY for this insert.
    """
    wants_copy = should_use_copy(config, row_count, write_mode, total_rows_hint=total_rows_hint)
    if wants_copy and not _frame_supports_copy(df):
        if config.bulk_insert_method == BulkInsertMethod.COPY:
            context.log.warning(
                "bulk_insert_method='copy' was requested, but this DataFrame "
                "has a nested-dtype column (List/Array/Struct), which "
                "df.write_csv cannot serialize for COPY. Falling back to the "
                "parameterized INSERT path, which handles nested types via "
                "psycopg's native list/array adaptation."
            )
        return False
    return wants_copy
...
# CSV encoding shared by every COPY-based insert: append/full_refresh
# (insert_with_copy) AND upsert staging (execute_upsert). Round-trips the
# three values the parameterized/executemany path preserves natively: SQL
# NULL, empty string, and the literal text "\N". quote_style="non_numeric"
# quotes every string field, so an unquoted empty field is unambiguously NULL
# (paired with COPY ... NULL '') while a quoted "" is the empty string and
# "\N" is literal text; numeric NULLs stay unquoted-empty => NULL. Verified
# empirically in docs/migrations/20260626_375-upsert-staging-merge.md (D1).
#
# insert_with_copy previously used its own encoding (null_value="\\N" paired
# with COPY ... NULL '\N'), which collapsed a literal two-character string
# "\N" to SQL NULL -- verified live during #456 review: a hinted batched
# append (routed to COPY) landed None for a "\N" value where the
# parameterized executemany path (routed to the same insert for a smaller
# hint) landed the literal string '\N' for the identical input. Both COPY
# call sites now share this encoding, so which insert path ``should_use_copy``
# / a ``total_rows_hint`` selects changes only statement shape, never the
# row values landed.
COPY_STAGING_OPTIONS = "FORMAT CSV, NULL ''"


def serialize_for_staging_copy(df: pl.DataFrame) -> bytes:
    r"""Serialize a DataFrame to CSV bytes for a COPY-based insert.

    Shared by every COPY call site -- append/full_refresh inserts
    (:func:`insert_with_copy`) and upsert staging (:func:`execute_upsert`).
    Uses the fidelity-preserving encoding pinned by migration 375 (D1): SQL
    NULL, empty string, and the literal text ``\N`` all round-trip
    distinctly, matching what the parameterized/executemany path preserves
    natively. Pair with ``COPY ... WITH (<COPY_STAGING_OPTIONS>)``.

    The name predates this function serving every COPY site (it originally
    served staging COPY only, per #375); kept as-is because existing tests
    import it by name.
    """
    buffer = BytesIO()
...
def insert_with_copy(
    ...
    r"""Insert DataFrame using PostgreSQL COPY protocol.
    ...
    Serialization uses :func:`serialize_for_staging_copy` /
    :data:`COPY_STAGING_OPTIONS` -- the same fidelity-preserving encoding
    upsert staging uses (#375 D1) -- so SQL NULL, empty string, and the
    literal text ``\N`` all round-trip distinctly, matching what the
    parameterized executemany path (:func:`insert_with_execute_values`)
    preserves natively. Before #456 review this function used its own
    ``null_value="\\N"`` encoding, which collapsed a literal ``\N`` string to
    SQL NULL; the two insert paths are now fidelity-equivalent, so which one
    ``should_use_copy`` / a streamed write's ``total_rows_hint`` selects
    changes only statement shape, never the row values landed.
    ...
    copy_sql = f"COPY {table_name} ({columns_str}) FROM STDIN WITH ({COPY_STAGING_OPTIONS})"  # noqa: S608

    chunk_size = get_effective_chunk_size(config, row_count)

    if chunk_size is None:
        # Single chunk -- DataFrame is small enough that serializing it in
        # one shot is acceptable, OR the operator explicitly disabled
        # chunking.
        with cursor.copy(copy_sql) as copy:
            copy.write(serialize_for_staging_copy(df))
    ...
    def _csv_chunks() -> Iterator[bytes]:
        ...
        for i, chunk_df in enumerate(df.iter_slices(n_rows=chunk_size)):
            chunk_bytes = serialize_for_staging_copy(chunk_df)
...
def insert_with_execute_values(
    ...
    underlying driver function.  Within each chunk, rows are further paged
    into multi-row statement executions per ``config.executemany_page_rows``
    (see :func:`_executemany_bulk`) to bound the number of pgaudit-logged
    statements the insert produces (#456).
    ...
    if chunk_size is None:
        # Process all at once
        rows = df.rows()
        _executemany_bulk(cursor, insert_sql, rows, page_rows=config.executemany_page_rows)
        return row_count
    else:
...
def insert_rows(
    ...
    *,
    ...
    Args:
        ...
        context: Dagster context for logging.
        total_rows_hint: Optional estimated total rows across the whole
            stream; forwarded to :func:`should_use_copy` so a streamed
            batched write sizes the COPY decision on the stream total rather
            than each batch alone (#456). ``None`` when the caller has no
            estimate (single-DataFrame callers never pass this).
    ...
    use_copy = _decide_use_copy(
        config, df, row_count, write_mode, context, total_rows_hint=total_rows_hint
    )
    if use_copy:
...
def execute_full_refresh(
    ...
    insert_method = "none"
    if row_count > 0:
        use_copy = _decide_use_copy(config, df, row_count, WriteMode.FULL_REFRESH, context)
        if use_copy:
...
def execute_append(
    ...
    use_copy = _decide_use_copy(config, df, row_count, WriteMode.APPEND, context)
    if use_copy:
        inserted_count = insert_with_copy(config, cursor, table_name, df, context)
        insert_method = "copy"
    else:
