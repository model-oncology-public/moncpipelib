...
class Period:
    ...
    Attributes:
        ...
        match: Cardinality of the ingest-boundary glob for this period.
            ``"one"`` (default) requires the glob to match exactly one
            landed file (drift detection); ``"many"`` permits N files
            (>=1) -- for sources delivered as N part files per partition
            (e.g. columnar parquet), where the resolver returns the
            ordered set instead of raising. See #438. Ignored for legacy
            (non-ingest) sources.
    """
...
class FromIngestTemplate:
    ...
    Attributes:
        ...
        effective_from_field: Name of the ingest manifest field that
            ...
            partition.
        match: Cardinality of the rendered glob per partition. ``"one"``
            (default) requires exactly one landed file; ``"many"`` permits
            N files (>=1), returning the ordered set -- for asset folders
            holding N part files per cycle (e.g. Trilliant parquet parts).
            See #438.
    """
