...
KNOWN_PERIOD_FIELDS: frozenset[str] = frozenset(
    {"source", "effective_from", "effective_to", "partition_key", "match"}
)
KNOWN_MATCH_MODES: frozenset[str] = frozenset({"one", "many"})
KNOWN_DATA_SOURCE_FIELDS: frozenset[str] = frozenset(
    {"source_id", "source_name", "description", "periods", "ingest_source"}
)
KNOWN_FROM_INGEST_TEMPLATE_FIELDS: frozenset[str] = frozenset(
    {"source", "effective_from_field", "match"}
)
...
KNOWN_INGEST_INNER_FIELDS: frozenset[str] = frozenset(
    {
        ...
        # Pattern-specific blocks are keyed by the pattern name (e.g.
        # "http_urls", "api_resolver") and are validated by the generic
        # loader via _PATTERN_BLOCK_VALIDATORS below -- an optional
        # validate_contract_block hook on the IngestPattern Protocol, so a
        # pattern owns its own contract validation, is the longer-term fix
        # and is out of scope here (see that map's comment). This set
        # deliberately carries no builtin pattern names: at validation
        # time it is unioned with the live pattern registry
        # (frozenset(INGEST_PATTERNS), #464), which always contains the
        # builtins because _register_builtin_patterns() runs at import --
        # so a literal list here could never be the reason a block key is
        # accepted, only a second source of truth to keep in sync by hand.
    }
)
...
# Supported values for ingest.<pattern>.idempotency (#464).
#
# Reserved extension point, not dead config: only ``http_urls`` and
# ``api_resolver`` accept this key today. Nothing in ``src/`` reads the
# value yet, so validating it only closes the typo hole (``hash_compre``
# previously passed). See the keep-vs-drop rationale and ADR lineage in
# ``docs/migrations/20260805_464-contract-validation-hardening.md``.
KNOWN_IDEMPOTENCY_MODES: frozenset[str] = frozenset({"hash_compare"})
...
# blob_mirror (#437): foreign-account source + SP credential + object filter.
KNOWN_BLOB_MIRROR_FIELDS: frozenset[str] = frozenset(
    {"source", "credential", "object_glob", "exclude_globs"}
)
KNOWN_BLOB_MIRROR_SOURCE_FIELDS: frozenset[str] = frozenset(
    {"account_url", "container", "object_prefix", "discovery_prefix", "partition_pattern"}
)
# blob_mirror's credential carries the cross-tenant SP identity: tenant_id
# + client_id are non-secret config; only the secret VALUE lives in Key
# Vault (referenced by secret_name).  See D2 in the #436-#439 migration.
KNOWN_BLOB_MIRROR_CREDENTIAL_FIELDS: frozenset[str] = frozenset(
    {"secret_name", "tenant_id", "client_id"}
)
# browser_export (#463): headless-browser-driven export plans.
KNOWN_BROWSER_EXPORT_FIELDS: frozenset[str] = frozenset(
    {
        "export_plan",
        "export_config",
        "allowed_hosts",
...
# No `idempotency`: browser_export offers no choice (D8) -- a daily-regenerated
# export differs byte-wise even when semantically unchanged, so hash-compare is
# the only mode and an inert knob would be a lie.
KNOWN_BROWSER_EXPORT_PARTITION_FIELDS: frozenset[str] = frozenset(
    {"mode", "cadence", "anchor_tz", "publication_lag_hours"}
)
# No `key_from`: there is no resolver release dict to read a key out of (D4).
KNOWN_BROWSER_EXPORT_PARTITION_MODES: frozenset[str] = frozenset({"dynamic"})
KNOWN_BROWSER_EXPORT_CADENCES: frozenset[str] = frozenset({"daily", "monthly", "quarterly"})
# `weekly` is absent deliberately: a weekly boundary needs an anchor day, and
# KNOWN_BROWSER_EXPORT_PARTITION_FIELDS carries no `anchor_dow` (#463 D4 fixes
# that set at four keys). ingest/_cadence.py::cadence_boundary still computes
# weekly for the calendar resolver -- this is a validation-surface restriction,
# not a helper limitation. See decision C1 in the #463 executor briefs.
KNOWN_BROWSER_EXPORT_BROWSER_FIELDS: frozenset[str] = frozenset(
    {"headless", "navigation_timeout_s", "download_timeout_s"}
)
KNOWN_BROWSER_EXPORT_FETCH_FIELDS: frozenset[str] = frozenset({"user_agent"})
# Deliberately NOT _COMMON_FETCH_FIELDS (D12): retries / timeout_s /
# connect_timeout_s are httpx-client knobs a browser cannot honor, and
# fetch.timeout_s would overlap browser.download_timeout_s with no stated
# precedence. Every browser timeout lives in the `browser` block.
KNOWN_BROWSER_EXPORT_VALIDATE_CONTENT_FIELDS: frozenset[str] = frozenset(
    {"reject_first_bytes_match", "max_first_bytes_check"}
)
# NOT KNOWN_VALIDATE_CONTENT_FIELDS, which admits content_type_in: a browser
# download exposes no Content-Type, and a content-type predicate would
# reject 100% of downloads (D7) -- see _validate_browser_export_block's
# dedicated rejection message for that key.
...
KNOWN_SINK_TABLE_FIELDS: frozenset[str] = frozenset(
    {
        ...
        "sequence_column",
        # Per-write full_refresh clear method override (auto/delete/truncate).
        # Pin "delete" for FK-referenced targets TRUNCATE cannot clear
        # (mirror issue model-oncology-public/moncpipelib#4).
        "full_refresh_method",
    }
)
...
# Valid values for the sink full_refresh_method override (avoids importing
# FullRefreshMethod from the IO manager to prevent a circular dependency).
# Must be kept in sync with the FullRefreshMethod enum.
KNOWN_FULL_REFRESH_METHODS: frozenset[str] = frozenset({"auto", "delete", "truncate"})

# Test modifier keys (valid alongside a test type in dict-form tests)
TEST_MODIFIERS: frozenset[str] = frozenset({"severity", "when"})

# Accepted values of the two test/expectation modifier keys above. Named
# constants (rather than set literals inlined at the _validate_enum_value
# call sites) so the allowed values live in one place alongside every
# other KNOWN_* enum in this module, and so the frozenset is built once at
# import instead of once per test, per column, per contract (a set literal
# passed as an argument is not folded to a constant the way `x in {...}` is).
KNOWN_TEST_SEVERITIES: frozenset[str] = frozenset({"error", "warn"})
KNOWN_TEST_WHEN_CONDITIONS: frozenset[str] = frozenset({"not_null"})
...
def _validate_enum_value(
    ...
    *,
    ...
    """Validate that ``container[key]``, if present, is one of ``known``.

    Shared by every "value must be one of this set" field in this module
    (idempotency modes, sensitivity, match modes, partition modes,
    test/expectation severity and ``when``) so the isinstance guard below
    is written -- and can be reviewed -- exactly once (#464/#467).

    ``subject`` is the error message's leading noun phrase, written out
    verbatim -- quoting included -- by the caller: ``"'sensitivity'"`` for
    a dotted field path, or ``"Period 0: 'match'"`` where the surrounding
    validator numbers its entries in prose and every sibling error reads
    that way. The caller owns the quoting precisely so a prose prefix is
    never wrapped in field-path quotes: a helper that quoted a passed-in
    ``path`` itself produced subjects like ``'Column 'id' test
    0.severity'``, nesting quotes and presenting prose as a dotted path.

    Returns ``[]`` when ``key`` is absent from ``container``: absence is
    the caller's business (a separate "is required" check, where one
    exists, handles it). When ``skip_falsy`` is set, a falsy present value
    (e.g. ``severity: ""``) is also treated as absent, preserving the
    pre-existing behavior at call sites that only ever validated a
    *truthy* value -- this must not start rejecting an empty string as a
    new behavior change.

    Otherwise returns exactly one error, never more: ``isinstance(value,
    str)`` is checked before the membership test because ``value in
    known`` calls ``hash(value)``, and a mapping or list value -- legal
    YAML a contract author can write -- is unhashable. A bare membership
    check would raise ``TypeError`` out of a function contracted to
    return an error list, crashing the loader instead of reporting a
    ...
    if not isinstance(value, str) or value not in known:
        return [f"{subject} must be one of {sorted(known)}, got {value!r}"]
    return []
...
def _validate_source_sink_entry(
    ...
    if str(entry_type) == "table":
        ...
        if label == "Sink":
            ...
            frm = entry.get("full_refresh_method")
            if frm is not None and not isinstance(frm, str):
                errors.append(
                    f"{prefix}: 'full_refresh_method' must be a string, got {type(frm).__name__}"
                )
            elif frm is not None and str(frm) not in KNOWN_FULL_REFRESH_METHODS:
                errors.append(
                    f"{prefix}: 'full_refresh_method' must be one of "
                    f"{sorted(KNOWN_FULL_REFRESH_METHODS)}, got '{frm}'"
                )
            elif (
                frm is not None
                and mode is not None
                and str(mode) in KNOWN_SINK_MODES
                and str(mode) != "full_refresh"
            ):
                # full_refresh_method is consumed only by the full_refresh clear
                # step; on any other declared mode it is inert config.
                errors.append(
                    f"{prefix}: 'full_refresh_method' is only valid with mode "
                    f"'full_refresh', got mode '{mode}'"
                )
...
def validate_data_source_schema(data: dict[str, Any]) -> list[str]:
    ...
    if "periods" not in data:
        ...
        for i, period in enumerate(data["periods"]):
            ...
            errors.extend(
                _validate_enum_value(period, "match", KNOWN_MATCH_MODES, f"{prefix}: 'match'")
            )
...
def _validate_from_ingest_periods(periods: dict[str, Any], data: dict[str, Any]) -> list[str]:
    ...
    errors.extend(
        _validate_enum_value(template, "match", KNOWN_MATCH_MODES, "'periods.template.match'")
    )
    return errors
...
def validate_ingest_contract_schema(data: dict[str, Any]) -> list[str]:
    ...
    Unknown ``ingest.pattern`` names are rejected here at load time
    (previously they failed first as a ``get_pattern`` ``KeyError`` at
    materialize time, well downstream of contract linting). Validation
    consults the live pattern registry (``INGEST_PATTERNS``), so a
    consumer-registered pattern must be registered (imported) before
    its contracts can be validated -- the registry snapshot is read at
    call time, not fixed to the builtin set.
    ...
    if sensitivity is None:
        ...
        errors.extend(
            _validate_enum_value(data, "sensitivity", KNOWN_SENSITIVITIES, "'sensitivity'")
        )

    # Attestation rule: phi / confidential contracts require data_owner + compliance_review.
    # Same isinstance guard as the shared enum helper above: `sensitivity
    # in {...}` would otherwise crash on an unhashable mapping/list value.
    if isinstance(sensitivity, str) and sensitivity in {"phi", "confidential"}:
    ...
    # Lazy import, same invariant as _validate_resolver_ref's `get_resolver`
    # and _validate_api_crawl_block's crawl-plan lookup: the contracts
    # package must not require the ingest package at module import time.
    # Keep it lazy.
    from moncpipelib.ingest.patterns import INGEST_PATTERNS, get_pattern

    known_patterns = frozenset(INGEST_PATTERNS)

    # Resolve `pattern` before the unknown-keys sweep (not after), so an
    # unregistered pattern's own block key can be excluded from it. The
    # "Unknown ingest pattern" error below already explains the real
    # problem; flagging `ingest.<pattern>` as an unknown field too is
    # false in substance -- it is the block key of an unregistered
    # pattern, not an illegal key -- and misdirects an author into
    # deleting the block instead of fixing `pattern:` or importing the
    # pattern module (#464/#467).
    pattern = ingest.get("pattern")
    known_inner_keys = KNOWN_INGEST_INNER_FIELDS | known_patterns
    if isinstance(pattern, str) and pattern not in known_patterns:
        known_inner_keys = known_inner_keys | {pattern}

    errors.extend(_check_unknown_keys(ingest, known_inner_keys, "'ingest'"))
    ...
    if pattern is None:
        ...
        # Reuse the membership answer already computed for the known-keys
        # widening above -- one source of truth for "is this pattern
        # registered" -- and call get_pattern purely to source the message,
        # so the registry keeps owning the wording (previously a
        # copy-pasted format string kept in sync only by a byte-equality
        # test). This loader adds the 'ingest.pattern' field path every
        # other loader error carries.
        detail = ""
        try:
            get_pattern(pattern)
        except KeyError as e:
            # e.args[0] is the raw message string; f"{e}" would add
            # KeyError's outer repr quotes and break the field-path prefix.
            detail = str(e.args[0]) if e.args else ""
        if not detail:
            # Defensive floor: we already know from `known_patterns` that
            # the name is unregistered, so an error is owed here no matter
            # what get_pattern did. Without this, a change to its failure
            # shape (a different exception type, or a KeyError with no
            # args) would silently reopen exactly the load-time hole #464
            # closes, and no test would notice.
            detail = f"Unknown ingest pattern {pattern!r}. Known patterns: {sorted(known_patterns)}"
        errors.append(f"'ingest.pattern': {detail}")
    ...
    # Pattern-specific validation, dispatched through
    # _PATTERN_BLOCK_VALIDATORS (defined below, once the validators it
    # references exist) rather than a hand-written if/elif chain -- see
    # that map's comment for why. Guard with isinstance: a non-str
    # pattern already has an error appended above and must not be used
    # to index the map.
    if isinstance(pattern, str):
        block_validator = _PATTERN_BLOCK_VALIDATORS.get(pattern)
        if block_validator is not None:
            errors.extend(block_validator(ingest.get(pattern)))
        elif (
            # A registered pattern with no dedicated block validator (a
            # consumer-registered custom pattern, #399): load_ingest_contract
            # still does `dict(ingest.get(pattern, {}))`, and a non-mapping
            # value there -- including an explicit YAML null -- raises
            # ValueError/TypeError instead of ContractValidationError.
            # Builtin patterns already get this exact message from their
            # own block validator above, so this is scoped to patterns
            # without one to avoid double-reporting (#464/#467).
            pattern in known_patterns
            and pattern in ingest
            and not isinstance(ingest.get(pattern), dict)
        ):
            errors.append(f"'ingest.{pattern}' must be a mapping")
        if pattern == "http_urls":
            # Genuine asymmetry: this check needs the whole `ingest` dict
            # (cross-references `extract` and
            # `payload_filename_template` alongside the http_urls block),
            # not just the block itself, so it cannot fit the map's
            # ``Callable[[Any], list[str]]`` signature. Kept as an
            # explicit adjunct after the generic dispatch rather than
            # contorting the map.
            errors.extend(_validate_non_archive_filename_uniqueness(ingest))
        if pattern == "browser_export":
            # Same asymmetry as the http_urls adjunct above: these need
            # the whole `ingest` dict (and, for the sensitivity rule, the
            # whole document), not just the pattern block, so they cannot
            # fit the map's ``Callable[[Any], list[str]]`` signature.
            errors.extend(_validate_browser_export_template_compat(ingest))
            if isinstance(sensitivity, str) and sensitivity != "public":
                errors.append(
                    "'sensitivity': pattern 'browser_export' is restricted to "
                    "sensitivity 'public'. A headless browser executing "
                    "arbitrary third-party JavaScript must not be pointed at "
                    f"a '{sensitivity}' source without a separate security "
                    "review (see SECURITY.md, 'Browser-Driven Ingest "
                    "Exports')."
                )
            if not data.get("compliance_review"):
                errors.append(
                    "'compliance_review' is required when pattern is "
                    "'browser_export': a headless browser in the ingest "
                    "runtime is a high-risk surface and needs a documented "
                    "pointer to its SECURITY.md entry, regardless of "
                    "sensitivity."
                )
...
def _validate_http_urls_block(block: Any) -> list[str]:
    ...
    errors.extend(
        _validate_enum_value(
            block, "idempotency", KNOWN_IDEMPOTENCY_MODES, "'ingest.http_urls.idempotency'"
        )
    )

    errors.extend(
        _validate_validate_content_block(
            block.get("validate_content"), KNOWN_VALIDATE_CONTENT_FIELDS, "ingest.http_urls"
        )
    )
...
def _validate_validate_content_block(
    block: Any, known: frozenset[str], block_path: str
) -> list[str]:
    """Validate an optional ``validate_content`` sub-block (#228, #463).

    Shared by ``http_urls`` (whitelist-then-blacklist against
    ``content_type_in`` + ``reject_first_bytes_match``) and
    ``browser_export`` (byte-match only -- a browser download exposes no
    Content-Type header, so ``content_type_in`` is deliberately excluded
    from ``known`` there and gets its own specific, actionable rejection
    message from the caller instead of falling through to this function's
    generic "unknown field").  ``known`` and ``block_path`` follow the
    same shape as :func:`_validate_fetch_block`.

    When set (either pattern), it switches the URL/download list
    semantics for the period from "process all as union" to "try in
    order, take first valid" for ``http_urls``. See the migration tracker
    at ``docs/migrations/20260426_228-validate-content-and-historical-release.md``.
    """
    errors: list[str] = []
    if block is None:
        return errors
    prefix = f"'{block_path}.validate_content'"
    if not isinstance(block, dict):
        errors.append(f"{prefix} must be a mapping")
        return errors

    errors.extend(_check_unknown_keys(block, known, prefix))
...
def _validate_api_resolver_block(block: Any) -> list[str]:
    ...
    errors.extend(
        _validate_enum_value(
            block, "idempotency", KNOWN_IDEMPOTENCY_MODES, "'ingest.api_resolver.idempotency'"
        )
    )
...
def _validate_resolver_ref(block: dict[str, Any], block_path: str) -> list[str]:
    ...
    if resolver_name is None:
        ...
        # package at module import time. Same invariant as the pattern
        # registry lookup in validate_ingest_contract_schema and the
        # crawl-plan lookup in _validate_api_crawl_block.
        ...
        if mode is None:
            ...
            errors.extend(
                _validate_enum_value(
                    partition,
                    "mode",
                    KNOWN_API_RESOLVER_PARTITION_MODES,
                    f"'{block_path}.partition.mode'",
                )
            )
...
def _validate_blob_mirror_block(block: Any) -> list[str]:
    """Validate the ``ingest.blob_mirror`` inner block (#437).

    Shape: a ``source`` sub-block naming the foreign account/container +
    object/discovery prefixes, an optional cross-tenant SP ``credential``
    sub-block, and object filters (``object_glob`` / ``exclude_globs``).
    """
    errors: list[str] = []
    if block is None:
        errors.append("'ingest.blob_mirror' is required when pattern is 'blob_mirror'")
        return errors
    if not isinstance(block, dict):
        errors.append("'ingest.blob_mirror' must be a mapping")
        return errors

    errors.extend(_check_unknown_keys(block, KNOWN_BLOB_MIRROR_FIELDS, "'ingest.blob_mirror'"))
    errors.extend(_validate_blob_mirror_source(block.get("source")))
    errors.extend(_validate_blob_mirror_credential(block.get("credential")))

    object_glob = block.get("object_glob")
    if object_glob is not None and (not isinstance(object_glob, str) or not object_glob):
        errors.append("'ingest.blob_mirror.object_glob' must be a non-empty string")

    exclude_globs = block.get("exclude_globs")
    if exclude_globs is not None and (
        not isinstance(exclude_globs, list)
        or not all(isinstance(g, str) and g for g in exclude_globs)
    ):
        errors.append("'ingest.blob_mirror.exclude_globs' must be a list of non-empty strings")

    return errors


def _validate_blob_mirror_source(source: Any) -> list[str]:
    """Validate the required ``blob_mirror.source`` sub-block."""
    errors: list[str] = []
    if source is None:
        errors.append("'ingest.blob_mirror.source' is required")
        return errors
    if not isinstance(source, dict):
        errors.append("'ingest.blob_mirror.source' must be a mapping")
        return errors

    errors.extend(
        _check_unknown_keys(source, KNOWN_BLOB_MIRROR_SOURCE_FIELDS, "'ingest.blob_mirror.source'")
    )
    for req in ("account_url", "container", "object_prefix"):
        value = source.get(req)
        if value is None:
            errors.append(f"'ingest.blob_mirror.source.{req}' is required")
        elif not isinstance(value, str) or not value:
            errors.append(f"'ingest.blob_mirror.source.{req}' must be a non-empty string")

    discovery_prefix = source.get("discovery_prefix")
    if discovery_prefix is not None and not isinstance(discovery_prefix, str):
        errors.append("'ingest.blob_mirror.source.discovery_prefix' must be a string")

    partition_pattern = source.get("partition_pattern")
    if partition_pattern is not None:
        if not isinstance(partition_pattern, str) or not partition_pattern:
            errors.append(
                "'ingest.blob_mirror.source.partition_pattern' must be a non-empty string"
            )
        else:
            import re as _re

            try:
                _re.compile(partition_pattern)
            except _re.error as e:
                errors.append(
                    f"'ingest.blob_mirror.source.partition_pattern' is not a valid regex: {e}"
                )
    return errors


def _validate_blob_mirror_credential(credential: Any) -> list[str]:
    """Validate the optional ``blob_mirror.credential`` sub-block.

    Optional (omit for local-dev ``DefaultAzureCredential``).  When
    present, the full SP triple is required: ``secret_name`` names the
    Key Vault secret holding the client secret; ``tenant_id`` (partner
    tenant) + ``client_id`` (our SP) are the non-secret identifiers a
    ``ClientSecretCredential`` needs.
    """
    errors: list[str] = []
    if credential is None:
        return errors
    if not isinstance(credential, dict):
        errors.append("'ingest.blob_mirror.credential' must be a mapping")
        return errors
    errors.extend(
        _check_unknown_keys(
            credential,
            KNOWN_BLOB_MIRROR_CREDENTIAL_FIELDS,
            "'ingest.blob_mirror.credential'",
        )
    )
    for req in ("secret_name", "tenant_id", "client_id"):
        value = credential.get(req)
        if value is None:
            errors.append(
                f"'ingest.blob_mirror.credential.{req}' is required when a "
                f"credential block is present"
            )
        elif not isinstance(value, str) or not value:
            errors.append(f"'ingest.blob_mirror.credential.{req}' must be a non-empty string")
    return errors


def _malformed_allowlist_host_reason(value: Any) -> str | None:
    """Return why ``value`` is not shaped like a bare ``allowed_hosts`` entry, else ``None``.

    Delegates to :func:`moncpipelib.ingest._hostnames.malformed_allowlist_host_reason`
    -- the ONE implementation of this check, also used as the
    materialize-time backstop in
    ``ingest/patterns/browser_export.py::materialize_partition`` (#463
    egress-allowlist reframe, Step 1b). This is the loader's ONLY per-entry
    ``allowed_hosts`` check: the shape pre-screen (bare hostname; no
    scheme, port, path, wildcard, percent-encoding, whitespace, malformed
    DNS label, or non-ASCII character).

    Lazy import: the ``contracts`` package must not require the ``ingest``
    package at module import time (same invariant as the resolver /
    crawl-plan / pattern-registry / export-plan lookups elsewhere in this
    module).
    """
    from moncpipelib.ingest._hostnames import malformed_allowlist_host_reason

    return malformed_allowlist_host_reason(value)


def _malformed_allowed_hosts_reason(value: Any) -> str | None:
    """Return why ``value`` is not shaped like a valid ``allowed_hosts`` list, else ``None``.

    Delegates to :func:`moncpipelib.ingest._hostnames.malformed_allowed_hosts_reason`
    -- the ONE implementation of this check, also used as the
    materialize-time backstop in
    ``ingest/patterns/browser_export.py::materialize_partition`` (#464/#468
    round 4, J6). The loader's LIST-level ``allowed_hosts`` check (must be
    a non-empty list), run before the per-entry
    :func:`_malformed_allowlist_host_reason` loop below -- moved here, per
    the same single-source discipline as that function, so the backstop
    cannot enforce a weaker guard (previously ``isinstance(..., list)``
    with no emptiness check) than the loader does.

    Lazy import: the ``contracts`` package must not require the ``ingest``
    package at module import time (same invariant as
    :func:`_malformed_allowlist_host_reason` above).
    """
    from moncpipelib.ingest._hostnames import malformed_allowed_hosts_reason

    return malformed_allowed_hosts_reason(value)


def _validate_browser_export_block(block: Any) -> list[str]:
    """Validate the ``ingest.browser_export`` inner block (#463).

    Structural shape: a registered ``export_plan`` name (dispatched to the
    plan's ``validate_config`` per ADR-2, mirroring
    ``_validate_api_crawl_block``'s ``crawl_plan`` handling), a required
    non-empty ``allowed_hosts`` list of bare hostnames (D5's allowlist;
    shape-validated only -- see
    ``docs/migrations/20260807_463-egress-allowlist-reframe.md``, round 6),
    a required ``partition`` block (daily/monthly/quarterly
    cadence only -- ``weekly`` is rejected with a specific reason, C1), an
    optional ``browser`` block (timeouts + ``headless``), a ``fetch``
    block restricted to a REQUIRED ``user_agent`` (D12 -- httpx-only knobs
    are rejected as unknown; ``user_agent`` itself is load-required rather
    than optional, #464/#468 review-gate finding 7.4 -- mirrors
    ``api_crawl.rate_limit_rps`` being load-required so a contract author
    must consciously supply it rather than silently inheriting chromium's
    default ``HeadlessChrome/...`` UA), and a ``validate_content`` block
    restricted to byte-match checks (``content_type_in`` gets a dedicated
    rejection message rather than a generic "unknown field", D7).
    """
    errors: list[str] = []
    if block is None:
        errors.append("'ingest.browser_export' is required when pattern is 'browser_export'")
        return errors
    if not isinstance(block, dict):
        errors.append("'ingest.browser_export' must be a mapping")
        return errors

    errors.extend(
        _check_unknown_keys(block, KNOWN_BROWSER_EXPORT_FIELDS, "'ingest.browser_export'")
    )

    # export_plan (required, string, registered) -- mirrors
    # _validate_api_crawl_block's crawl_plan handling exactly.
    plan_name = block.get("export_plan")
    plan = None
    if plan_name is None:
        errors.append("'ingest.browser_export.export_plan' is required")
    elif not isinstance(plan_name, str):
        errors.append("'ingest.browser_export.export_plan' must be a string")
    else:
        # Lazy import: the contracts package must not require the ingest
        # package at module import time (same invariant as the resolver /
        # crawl-plan / pattern-registry lookups above).
        from moncpipelib.ingest.export_plans import get_export_plan

        try:
            plan = get_export_plan(plan_name)
        except KeyError as e:
            errors.append(f"'ingest.browser_export.export_plan': {e}")

    # export_config dispatched to the plan's validate_config (ADR-2).
    export_cfg = block.get("export_config")
    if export_cfg is None:
        export_cfg = {}
    elif not isinstance(export_cfg, dict):
        errors.append("'ingest.browser_export.export_config' must be a mapping")
        export_cfg = {}
    if plan is not None and isinstance(export_cfg, dict):
        errors.extend(
            f"'ingest.browser_export.export_config.{e}'" for e in plan.validate_config(export_cfg)
        )

    # allowed_hosts (D5): required, non-empty list of bare hostnames.
    allowed_hosts = block.get("allowed_hosts")
    if allowed_hosts is None:
        errors.append(
            "'ingest.browser_export.allowed_hosts' is required: a browser_export "
            "contract must declare every host the page may reach"
        )
    else:
        list_shape_reason = _malformed_allowed_hosts_reason(allowed_hosts)
        if list_shape_reason is not None:
            errors.append(f"'ingest.browser_export.allowed_hosts' {list_shape_reason}")
        else:
            for i, h in enumerate(allowed_hosts):
                shape_reason = _malformed_allowlist_host_reason(h)
                if shape_reason is not None:
                    errors.append(
                        f"'ingest.browser_export.allowed_hosts[{i}]' {shape_reason}; got {h!r}"
                    )

    # partition (D4's #452 guard): required.
    partition = block.get("partition")
    if partition is None:
        errors.append("'ingest.browser_export.partition' is required")
    elif not isinstance(partition, dict):
        errors.append("'ingest.browser_export.partition' must be a mapping")
    else:
        errors.extend(
            _check_unknown_keys(
                partition,
                KNOWN_BROWSER_EXPORT_PARTITION_FIELDS,
                "'ingest.browser_export.partition'",
            )
        )
        if "mode" not in partition:
            errors.append("'ingest.browser_export.partition.mode' is required")
        else:
            errors.extend(
                _validate_enum_value(
                    partition,
                    "mode",
                    KNOWN_BROWSER_EXPORT_PARTITION_MODES,
                    "'ingest.browser_export.partition.mode'",
                )
            )
        if "cadence" not in partition:
            errors.append("'ingest.browser_export.partition.cadence' is required")
        else:
            errors.extend(
                _validate_enum_value(
                    partition,
                    "cadence",
                    KNOWN_BROWSER_EXPORT_CADENCES,
                    "'ingest.browser_export.partition.cadence'",
                )
            )
            if partition.get("cadence") == "weekly":
                errors.append(
                    "'ingest.browser_export.partition.cadence': 'weekly' is not "
                    "supported by browser_export -- a weekly boundary requires an "
                    "anchor day-of-week, and this pattern's partition block "
                    "carries no 'anchor_dow' field. Use 'daily', 'monthly', or "
                    "'quarterly'."
                )
        if "anchor_tz" not in partition:
            errors.append(
                "'ingest.browser_export.partition.anchor_tz' is required: a "
                "daily cadence boundary is timezone-sensitive, and an unstated "
                "zone silently shifts every partition key relative to the "
                "publisher's own calendar"
            )
        else:
            anchor_tz = partition["anchor_tz"]
            if not isinstance(anchor_tz, str) or not anchor_tz:
                errors.append(
                    "'ingest.browser_export.partition.anchor_tz' must be a "
                    "non-empty IANA timezone name"
                )
            else:
                # Imported locally to keep loader.py's module-level imports
                # unchanged (per the #463 executor briefs).
                from zoneinfo import ZoneInfo, ZoneInfoNotFoundError

                try:
                    ZoneInfo(anchor_tz)
                except (ZoneInfoNotFoundError, ValueError):
                    # ZoneInfo raises ValueError (not just
                    # ZoneInfoNotFoundError) on a malformed key (e.g. one
                    # containing ".."); a validator that returns error
                    # lists must never raise, so both are caught here.
                    # calendar.py catches only ZoneInfoNotFoundError today
                    # -- a pre-existing gap, out of scope for this change.
                    errors.append(
                        f"'ingest.browser_export.partition.anchor_tz': unknown "
                        f"timezone {anchor_tz!r} (must be an IANA timezone name)"
                    )
        if "publication_lag_hours" in partition:
            lag = partition["publication_lag_hours"]
            if not isinstance(lag, int) or isinstance(lag, bool) or lag < 0:
                errors.append(
                    "'ingest.browser_export.partition.publication_lag_hours' "
                    "must be a non-negative integer (hours)"
                )
    ...
    if browser is not None:
        if not isinstance(browser, dict):
            errors.append("'ingest.browser_export.browser' must be a mapping")
        else:
            errors.extend(
                _check_unknown_keys(
                    browser,
                    KNOWN_BROWSER_EXPORT_BROWSER_FIELDS,
                    "'ingest.browser_export.browser'",
                )
            )
            if "headless" in browser and not isinstance(browser["headless"], bool):
                # headless: false is accepted -- a debugging affordance,
                # not a security control; rejecting it at load time would
                # make local reproduction impossible.
                errors.append("'ingest.browser_export.browser.headless' must be a boolean")
            for key in ("navigation_timeout_s", "download_timeout_s"):
                if key in browser:
                    value = browser[key]
                    if not isinstance(value, (int, float)) or isinstance(value, bool) or value <= 0:
                        errors.append(
                            f"'ingest.browser_export.browser.{key}' must be a positive number"
                        )

    # fetch: restricted to user_agent (D12) -- reusing the shared
    # validator gives the non-empty/printable-ASCII checks for free and
    # makes retries/timeout_s/connect_timeout_s unknown fields rather than
    # silent no-ops.
    fetch_block = block.get("fetch")
    errors.extend(
        _validate_fetch_block(
            fetch_block, KNOWN_BROWSER_EXPORT_FETCH_FIELDS, "ingest.browser_export"
        )
    )
    # user_agent is load-required, not merely validated when present
    # (#464/#468 review-gate finding 7.4): `browser_export.py` passes
    # `user_agent=None` to `browser_session()` when unset, so the session
    # runs with chromium's default `HeadlessChrome/...` UA -- SECURITY.md
    # names "a descriptive User-Agent" as part of this pattern's access
    # posture, and that claim is only true if a contract cannot omit it.
    # Mirrors `api_crawl.rate_limit_rps` being load-required for the same
    # reason: a contract author must consciously supply the value, not
    # silently inherit a default. `_validate_fetch_block` above already
    # type/format-checks a *present* `user_agent`; this only checks for
    # its absence, so the two checks never both fire for the same cause.
    if fetch_block is None or (isinstance(fetch_block, dict) and "user_agent" not in fetch_block):
        errors.append(
            "'ingest.browser_export.fetch.user_agent' is required: a "
            "browser_export session must identify itself with a "
            "descriptive User-Agent rather than chromium's default "
            "'HeadlessChrome/...' string (see SECURITY.md, "
            "'Browser-Driven Ingest Exports')."
        )

    # validate_content: byte-match only; content_type_in gets its own
    # actionable rejection rather than falling through to a generic
    # "unknown field" (D7 -- a browser download exposes no Content-Type).
    errors.extend(
        _validate_validate_content_block(
            block.get("validate_content"),
            KNOWN_BROWSER_EXPORT_VALIDATE_CONTENT_FIELDS | {"content_type_in"},
            "ingest.browser_export",
        )
    )
    vc = block.get("validate_content")
    if isinstance(vc, dict) and "content_type_in" in vc:
        errors.append(
            "'ingest.browser_export.validate_content.content_type_in' is not "
            "supported: a browser download exposes no Content-Type header, so "
            "a content-type predicate would reject every download. Use "
            "'reject_first_bytes_match' (byte-based) instead."
        )

    return errors


def _validate_browser_export_template_compat(ingest: dict[str, Any]) -> list[str]:
    """Reject ``payload_filename_template`` for a multi-file export plan (#463 D8).

    Needs the whole ``ingest`` dict -- ``payload_filename_template`` is a
    top-level key while the plan name lives in the pattern block -- so it
    cannot fit ``_PATTERN_BLOCK_VALIDATORS``'s ``Callable[[Any], list[str]]``
    signature. Kept as an explicit adjunct after the generic dispatch,
    exactly like ``_validate_non_archive_filename_uniqueness`` for
    ``http_urls``.
    """
    if "payload_filename_template" not in ingest:
        return []
    browser_export_block = ingest.get("browser_export")
    if not isinstance(browser_export_block, dict):
        return []
    plan_name = browser_export_block.get("export_plan")
    if not isinstance(plan_name, str) or not plan_name:
        return []

    # Lazy import, same invariant as every other ingest-package lookup in
    # this module.
    from moncpipelib.ingest.export_plans import get_export_plan
    ...
    try:
        ...
        # Never double-report an unregistered plan --
        # _validate_browser_export_block already did.
        return []

    if getattr(plan, "multi_file", False):
        return [
            f"'ingest.payload_filename_template' is not valid with export plan "
            f"{plan_name!r}: the plan declares multi_file=True, and one "
            f"template renders the same filename for every file it yields -- "
            f"the later files would overwrite the earlier ones while "
            f"'_manifest.json' recorded all of them. Remove the template so "
            f"each file names itself from its sanitized suggested_filename."
        ]
    return []


# Dispatch map from ``ingest.pattern`` name to its block validator (#464).
#
# Looked up by ``validate_ingest_contract_schema`` instead of a
# hand-written if/elif chain: a chain has to be kept in sync by hand
# with every builtin pattern's name, and a forgotten branch silently
# skips block validation -- the contract loads clean and the malformed
# block only fails at materialize time, exactly the failure class #464
# exists to eliminate. A missing or misspelled entry in this map is
# visible at read/review time; a forgotten ``elif`` is not.
#
# A pattern registered in ``INGEST_PATTERNS`` with no entry here is
# intentionally left block-unvalidated -- the same gap as a
# consumer-registered custom pattern's block being accepted unchecked
# (see ``test_registered_custom_pattern_and_block_accepted``). The real
# long-term fix is an optional ``validate_contract_block`` hook on the
# ``IngestPattern`` Protocol so a pattern owns its own contract
# validation; that is a Protocol change and out of scope here.
_PATTERN_BLOCK_VALIDATORS: Mapping[str, Callable[[Any], list[str]]] = {
    "http_urls": _validate_http_urls_block,
    "api_resolver": _validate_api_resolver_block,
    "api_crawl": _validate_api_crawl_block,
    "blob_mirror": _validate_blob_mirror_block,
    "browser_export": _validate_browser_export_block,
}
...
def _validate_cross_contracts(
    ...
    for name, source in sources.items():
        ...
        if isinstance(source.periods, FromIngestTemplate):
            if not _ingest_produces_dynamic_partitions(ingest):
                partition_mode = (
                ...
                errors.append(
                    ...
                    f"linked ingest '{ingest.source_name}' to produce dynamically "
                    f"discovered partitions -- either by declaring "
                    f"'partition.mode: dynamic' or by using the 'blob_mirror' "
                    f"pattern (folder-walk discovery) -- but pattern "
                    f"'{ingest.pattern}' declared partition mode '{partition_mode}'"
                )
        else:
...
def _ingest_produces_dynamic_partitions(ingest: IngestContract) -> bool:
    """Return whether ``ingest`` produces dynamically-discovered partitions.

    True when EITHER:

    - The pattern config declares ``partition.mode: dynamic`` (the
      ``api_resolver`` / ``api_crawl`` shape: a resolver enumerates
      partitions at run time, and the contract records that fact in an
      explicit ``partition`` block).
    - ``ingest.pattern == "blob_mirror"``. blob_mirror (#437) has no
      ``partition`` config key at all -- its field allowlist
      (``KNOWN_BLOB_MIRROR_FIELDS`` above) doesn't include one, because
      there is no static period list to declare a mode against in the
      first place. Discovery happens by folder-walk against the foreign
      blob store (``BlobMirrorPattern.discover_partitions``), which is
      inherently dynamic, so blob_mirror qualifies unconditionally.

    Used by ``_validate_cross_contracts`` to decide whether a
    ``periods.mode: from_ingest`` DataSource may link to ``ingest``.
    """
    if ingest.pattern == "blob_mirror":
        return True
    partition = ingest.pattern_config.get("partition")
    partition_mode = partition.get("mode") if isinstance(partition, dict) else None
    return partition_mode == "dynamic"
...
def _build_contract_index(
    ...
    for search_dir in search_paths:
        ...
        for contract_file in sorted(search_dir.rglob(CONTRACT_FILE_PATTERN)):
            ...
            # Validate layer value against VALID_LAYERS. The isinstance
            # guard must run first: a mapping or list value is unhashable,
            # and `x not in <frozenset>` calls hash(x) before the
            # membership test -- a bare membership check would raise
            # TypeError here instead of the intended ContractValidationError
            # (#464/#467).
            if contract_layer is not None and (
                not isinstance(contract_layer, str) or contract_layer not in VALID_LAYERS
            ):
                raise ContractValidationError(
...
def validate_contract_schema(data: dict[str, Any]) -> list[str]:
    ...
    # Check version. The isinstance guard runs first for the same reason
    # as in _validate_enum_value: `x not in <set>` calls hash(x), and a
    # mapping or list value -- legal YAML a contract author can write --
    # is unhashable, so a bare membership test raised TypeError out of a
    # function contracted to return an error list (#467). Not routed
    # through the shared helper because this field owns a
    # differently-worded message that callers assert on.
    version = data.get("version")
    if version and (not isinstance(version, str) or version not in SUPPORTED_VERSIONS):
        errors.append(f"Unsupported contract version '{version}'. Supported: {SUPPORTED_VERSIONS}")

    # Layer must be one of VALID_LAYERS when present (#464/#467). Mirrors
    # the hard-fail already enforced at contract-index build time
    # (_build_contract_index), which only runs for index-resolved
    # contracts; load_contract previously emitted no layer error at all
    # for a bad value. See the exact compatibility delta (what's newly
    # rejected vs. what was already rejected elsewhere) in
    # docs/migrations/20260805_464-contract-validation-hardening.md.
    errors.extend(_validate_enum_value(data, "layer", VALID_LAYERS, "'layer'"))
...
def _validate_column(col: Any, index: int) -> list[str]:
    ...
    if not col_type:
        ...
        # isinstance first: `col_type not in valid_types` calls
        # hash(col_type), so a mapping or list value raised TypeError out
        # of a function contracted to return an error list (#467).
        if not isinstance(col_type, str) or col_type not in valid_types:
...
def _validate_test(test: Any, prefix: str) -> list[str]:
    ...
    if not test_keys:
        ...
        # test_type is deliberately str()-coerced (not routed through
        # _validate_enum_value) so a non-string dict key still stringifies
        # to a readable "unknown test type" message instead of a
        # membership-test crash; do not "fix" this to the shared helper.
    ...
    errors.extend(
        _validate_enum_value(
            test, "severity", KNOWN_TEST_SEVERITIES, f"{prefix}: 'severity'", skip_falsy=True
        )
    )
    ...
    errors.extend(
        _validate_enum_value(
            test, "when", KNOWN_TEST_WHEN_CONDITIONS, f"{prefix}: 'when'", skip_falsy=True
        )
    )
...
def _validate_expectation(exp: Any, index: int) -> list[str]:
    ...
    if not exp_keys:
        ...
        # exp_type is deliberately str()-coerced (not routed through
        # _validate_enum_value) so a non-string dict key still stringifies
        # to a readable "unknown expectation type" message instead of a
        # membership-test crash; do not "fix" this to the shared helper.
    ...
    errors.extend(
        _validate_enum_value(
            exp, "severity", KNOWN_TEST_SEVERITIES, f"{prefix}: 'severity'", skip_falsy=True
        )
    )
...
def _parse_periods(
    ...
    if isinstance(raw, dict):
        ...
        return FromIngestTemplate(
            ...
            match=str(template.get("match", "one")),  # type: ignore[arg-type]
        )
    if not raw:
        return []
    periods: list[Period] = []
    for entry in raw:
        ...
        periods.append(
            Period(
                ...
                match=str(entry.get("match", "one")),  # type: ignore[arg-type]
            )
        )
    return periods
