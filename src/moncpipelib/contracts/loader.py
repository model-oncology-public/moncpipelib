...
def validate_data_source_schema(data: dict[str, Any]) -> list[str]:
    ...
    if "periods" not in data:
        ...
        for i, period in enumerate(data["periods"]):
            ...
            if "match" in period and period["match"] not in KNOWN_MATCH_MODES:
                errors.append(
                    f"{prefix}: 'match' must be one of {sorted(KNOWN_MATCH_MODES)}, "
                    f"got {period['match']!r}"
                )
...
def _validate_from_ingest_periods(periods: dict[str, Any], data: dict[str, Any]) -> list[str]:
    ...
    if "match" in template and template["match"] not in KNOWN_MATCH_MODES:
        errors.append(
            f"'periods.template.match' must be one of {sorted(KNOWN_MATCH_MODES)}, "
            f"got {template['match']!r}"
        )
    return errors
...
def validate_ingest_contract_schema(data: dict[str, Any]) -> list[str]:
    ...
    elif pattern == "blob_mirror":
        errors.extend(_validate_blob_mirror_block(ingest.get("blob_mirror")))
