...
class TestWriteChanged:
    ...
    def test_partition_clear_with_empty_reload_is_a_change(self) -> None:
        """Batched partition-scoped clear feeds rows_deleted (no clear_method).

        A partition DELETE that removed rows but reloaded nothing this run
        (rows_deleted > 0, rows_inserted == 0, row_count == 0) must still count
        as a change so the emptied partition's stats get refreshed -- the
        counter, not clear_method, carries the signal on this branch (#4).
        """
        assert _write_changed({"rows_deleted": 5000, "rows_inserted": 0}, row_count=0) is True
