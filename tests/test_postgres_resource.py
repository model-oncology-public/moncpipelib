...
# ---------------------------------------------------------------------------
# TestGetWriterConfigValidation
# ---------------------------------------------------------------------------


class TestGetWriterConfigValidation:
    """#456 review Fix 5: ``executemany_page_rows`` validated in
    ``_get_writer_config`` before it reaches ``_executemany_bulk``.

    Unlike ``insert_chunk_size``, ``0`` does not mean "disabled" for this
    field -- ``_executemany_bulk`` has no unpaged mode, and per-row emission
    (page_rows=1) is exactly the pgaudit-volume problem #456 fixed. A
    non-positive value is always a misconfiguration.
    """

    @pytest.mark.parametrize("invalid_page_rows", [0, -5])
    def test_non_positive_page_rows_raises(
        self, resource: PostgresResource, invalid_page_rows: int
    ) -> None:
        bad_resource = resource.model_copy(update={"executemany_page_rows": invalid_page_rows})
        with pytest.raises(ValueError, match="executemany_page_rows"):
            bad_resource._get_writer_config()

    def test_default_page_rows_is_valid(self, resource: PostgresResource) -> None:
        """Sanity: the resource's own default never trips the guard."""
        config = resource._get_writer_config()
        assert config.executemany_page_rows == resource.executemany_page_rows
    ...
    def test_batched_write_injects_partition_column_from_contract_when_caller_omits(
        ...
        with (
            ...
            patch(
                ...
                side_effect=lambda _config, _cursor, _table, batch_df, _mode, _wctx, **_kw: (
    ...
    def test_batched_write_explicit_partition_column_kwarg_still_works(
        ...
        with (
            ...
            patch(
                "moncpipelib.io_managers.writers.insert_rows",
                side_effect=lambda _config, _cursor, _table, batch_df, _mode, _wctx, **_kw: (
                    landed.append(list(batch_df.columns))
                ),
            ),
        ):
            write_resource.write(
...
class TestPartitionColumnNullGuardWiring:
    """#480: prove ``_validate_partition_column_values`` is wired on BOTH the
    single-DataFrame and batched write paths, before any partition-scoped
    DELETE reaches the database.

    ``validate_partition_column_values`` itself is unit-tested directly
    (no database) in ``tests/test_partition_column_guards.py``. These tests
    prove the resource actually calls it at the right point on each path.
    """
    ...
    @patch("moncpipelib.resources.postgres.psycopg.connect")
    def test_single_path_null_partition_value_raises_before_delete(
        self,
        mock_connect: MagicMock,
        write_resource: PostgresResource,
    ) -> None:
        """A NULL in a declared partition_column raises before any DELETE."""
        from moncpipelib.contracts.exceptions import ContractViolationError
        from moncpipelib.resources.types import WriteContext
        ...
        wctx = WriteContext(asset_name="dim_provider", run_id="run-null-single", log=MagicMock())
        df = pl.DataFrame({"id": [1, 2], "load_period": ["2026-01", None]})

        with pytest.raises(ContractViolationError, match="contains NULLs"):
            write_resource.write(
                df,
                target="silver.dim_provider",
                context=wctx,
        ...
        assert not any("DELETE FROM" in s for s in sqls), (
            f"no DELETE should have reached the cursor before the NULL-partition "
            f"guard raised; executed: {sqls}"
        )

    @patch("moncpipelib.resources.postgres.psycopg.connect")
    def test_batched_path_null_in_second_batch_raises(
        self,
        mock_connect: MagicMock,
        write_resource: PostgresResource,
    ) -> None:
        """A NULL in batch 1's partition column raises even though batch 0 is
        clean. A guard placed under ``if i == 0`` would pass batch 0 and
        never see batch 1 -- this is the load-bearing assertion (#480)."""
        from moncpipelib.contracts.exceptions import ContractViolationError
        from moncpipelib.resources.types import WriteContext
        from moncpipelib.streaming import BatchedDataFrame
        ...
        df1 = pl.DataFrame({"id": ["a"], "load_period": ["2024-02-01"]})
        df2 = pl.DataFrame({"id": ["b"], "load_period": [None]})
        batched = BatchedDataFrame(batches=iter([df1, df2]), total_rows_hint=2)

        wctx = WriteContext(
            asset_name="reference_bronze/batched_null_table",
            run_id="run-null-batched",
            log=MagicMock(),
            has_partition_key=True,
            partition_keys=["2024-02-01"],
        )
        ...
        with (
            patch.object(PostgresResource, "_validate_columns"),
            pytest.raises(ContractViolationError, match="contains NULLs"),
        ):
            write_resource.write(
                batched,
                target="reference_bronze.batched_null_table",
                context=wctx,
                write_mode="append",
                partition_column="load_period",
                contract=None,
    ...
    @patch("moncpipelib.resources.postgres.psycopg.connect")
    def test_batched_path_null_raises_before_partition_delete(
        self,
        mock_connect: MagicMock,
        write_resource: PostgresResource,
    ) -> None:
        """A NULL in batch 0's partition column must raise before the
        inline partition-scoped ``DELETE`` in the ``i == 0`` prep block
        (``full_refresh`` + non-empty ``partition_keys``) ever reaches the
        cursor. The guard runs immediately after inject, on every batch;
        the DELETE fires later, inside ``if i == 0`` (postgres.py ~3347).
        Only a NULL in batch 0 -- not batch 1 -- can prove the guard
        precedes that DELETE, since the DELETE has already executed by the
        time batch 1 is processed."""
        from moncpipelib.contracts.exceptions import ContractViolationError
        from moncpipelib.resources.types import WriteContext
        from moncpipelib.streaming import BatchedDataFrame
        ...
        df1 = pl.DataFrame({"id": ["a"], "load_period": [None]})
        df2 = pl.DataFrame({"id": ["b"], "load_period": ["2024-02-01"]})
        batched = BatchedDataFrame(batches=iter([df1, df2]), total_rows_hint=2)

        wctx = WriteContext(
            asset_name="reference_bronze/batched_null_table",
            run_id="run-null-batched-before-delete",
            log=MagicMock(),
            has_partition_key=True,
            partition_keys=["2024-02-01"],
        )
        ...
        sqls = [str(c[0][0]) for c in mock_cursor.execute.call_args_list]
        assert not any("DELETE FROM" in s for s in sqls), (
            f"no partition-scoped DELETE should have reached the cursor before "
            f"the NULL-partition guard raised; executed: {sqls}"
        )

    @patch("moncpipelib.resources.postgres.psycopg.connect")
    def test_single_path_missing_partition_column_raises_contract_violation(
        self,
        mock_connect: MagicMock,
        write_resource: PostgresResource,
    ) -> None:
        """A declared partition_column absent from the DataFrame must raise
        ``ContractViolationError`` (the #480 guard's message), not the bare
        ``ValueError`` from ``_validate_write_config``. Pre-fix, the single
        path ran ``_validate_write_config`` before the guard, so the
        ValueError fired first and shadowed the guard's detailed message."""
        from moncpipelib.contracts.exceptions import ContractViolationError
        from moncpipelib.resources.types import WriteContext
        ...
        wctx = WriteContext(asset_name="dim_provider", run_id="run-missing-single", log=MagicMock())
        df = pl.DataFrame({"id": [1, 2]})

        with pytest.raises(ContractViolationError, match="does not contain that column"):
            write_resource.write(
                df,
                target="silver.dim_provider",
                context=wctx,
    ...
    @patch("moncpipelib.resources.postgres.psycopg.connect")
    def test_batched_path_missing_partition_column_raises_contract_violation(
        self,
        mock_connect: MagicMock,
        write_resource: PostgresResource,
    ) -> None:
        """Mirror of the single-path test above: the batched path must
        raise the same ``ContractViolationError`` for a declared
        partition_column absent from the DataFrame, proving the two write
        paths agree on exception type for this defect."""
        from moncpipelib.contracts.exceptions import ContractViolationError
        from moncpipelib.resources.types import WriteContext
        from moncpipelib.streaming import BatchedDataFrame
        ...
        wctx = WriteContext(
            asset_name="reference_bronze/batched_missing_table",
            run_id="run-missing-batched",
            log=MagicMock(),
        )

        with (
            patch.object(PostgresResource, "_validate_columns"),
            pytest.raises(ContractViolationError, match="does not contain that column"),
        ):
            write_resource.write(
                batched,
                target="reference_bronze.batched_missing_table",
                context=wctx,
                write_mode="append",
                partition_column="load_period",
                contract=None,
