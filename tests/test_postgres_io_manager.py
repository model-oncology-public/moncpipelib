...
class TestReconcileFullRefreshMethod:
    """Tests for full_refresh_method reconciliation (four-way pattern, #4).

    ``full_refresh_method`` reconciles to ``str | None``: ``None`` means
    neither the contract nor the ``write()`` kwarg set it, so the resource
    default applies downstream in ``_get_writer_config``.
    """
    ...
    @staticmethod
    def _contract(method: str = "delete") -> DataContract:
        return DataContract(
            version="1.0",
            pipeline_id="test-pipeline",
            asset="claims_837i",
            ...
            sinks=[
                {
                    ...
                    "table": "claims_837i",
                    "mode": "full_refresh",
                    "full_refresh_method": method,
                }
            ],
        )

    def test_neither_set_returns_none(self, mock_context):
        """No contract, no kwarg -> None, so the resource default wins later."""
        from moncpipelib.contracts.reconciliation import ContractReconciler

        result = ContractReconciler.reconcile_full_refresh_method(
            None,
            "claims_837i",
            {"full_refresh_method": None, "full_refresh_method_explicit": False},
            mock_context,
        )
        assert result is None

    def test_contract_only_overrides_default(self, mock_context):
        """Contract sink value silently overrides the (None) default."""
        from moncpipelib.contracts.reconciliation import ContractReconciler

        result = ContractReconciler.reconcile_full_refresh_method(
            self._contract("delete"),
            "claims_837i",
            {"full_refresh_method": None, "full_refresh_method_explicit": False},
            mock_context,
        )
    ...
    def test_kwarg_only_unchanged_when_no_sink_match(self, mock_context):
        """write() kwarg is used when the contract has no matching sink."""
        from moncpipelib.contracts.reconciliation import ContractReconciler

        result = ContractReconciler.reconcile_full_refresh_method(
            None,
            "claims_837i",
            {"full_refresh_method": "truncate", "full_refresh_method_explicit": True},
            mock_context,
        )
    ...
    def test_both_same_value_logs_warning(self, mock_context):
        ...
        result = ContractReconciler.reconcile_full_refresh_method(
            self._contract("delete"),
            "claims_837i",
            {"full_refresh_method": "delete", "full_refresh_method_explicit": True},
            mock_context,
        )
        assert result == "delete"
        mock_context.log.warning.assert_called_once()
    ...
    def test_different_values_raises_error(self, mock_context):
        ...
        with pytest.raises(ContractViolationError, match="full_refresh_method conflict"):
            ContractReconciler.reconcile_full_refresh_method(
                self._contract("delete"),
                "claims_837i",
                {"full_refresh_method": "truncate", "full_refresh_method_explicit": True},
                mock_context,
