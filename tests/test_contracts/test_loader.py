...
class TestContractValidation:
    ...
    def test_invalid_layer_value_rejected_by_schema_validation(self):
        """A layer value outside VALID_LAYERS must be caught by schema
        validation directly (#464/#467), not only when a contract is
        resolved by asset name through the contract index -- load_contract
        calls validate_contract_schema directly and previously emitted no
        layer error at all for a bad value."""
        data = {
            "version": "1.0",
            "pipeline_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
            "asset": "test",
            "layer": "reference_silver",
            "schema": {"columns": [{"name": "id", "type": "integer", "nullable": False}]},
        }
        errors = validate_contract_schema(data)
        assert any("'layer' must be one of" in e for e in errors)

    def test_layer_mapping_value_rejected_not_raised(self):
        """A mapping layer value is unhashable, and a bare membership test
        against VALID_LAYERS would raise TypeError instead of returning a
        clean error."""
        data = {
            "version": "1.0",
            "pipeline_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
        ...
        errors = validate_contract_schema(data)
        layer_errors = [e for e in errors if "'layer'" in e]
        assert len(layer_errors) == 1
        assert "must be one of" in layer_errors[0]

    @pytest.mark.parametrize("bad", [{"a": "b"}, ["2.0"]], ids=["mapping", "list"])
    def test_unsupported_version_unhashable_value_rejected_not_raised(self, bad):
        """`version not in SUPPORTED_VERSIONS` calls hash(version), so a
        mapping or list value -- legal YAML -- raised TypeError out of a
        function contracted to return an error list (#467). Must report
        cleanly instead."""
        data = {
            "version": bad,
            "pipeline_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
    ...
    @pytest.mark.parametrize("bad", [{"a": "b"}, ["integer"]], ids=["mapping", "list"])
    def test_column_type_unhashable_value_rejected_not_raised(self, bad):
        """Same defect on the per-column `type` membership test (#467):
        `col_type not in valid_types` hashes the value."""
        data = {
            "version": "1.0",
            "pipeline_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
            "asset": "test",
            "layer": "bronze",
            "schema": {"columns": [{"name": "id", "type": bad, "nullable": False}]},
        }
        errors = validate_contract_schema(data)
...
class TestLayerAwareContractIndex:
    ...
    def test_invalid_layer_mapping_value_raises_not_crashes(self, tmp_path):
        """A mapping layer value is unhashable; the isinstance guard must
        run before the membership test so this raises
        ContractValidationError instead of TypeError (#464/#467)."""
        (tmp_path / "bad.contract.yaml").write_text(
            _LAYERED_CONTRACT_YAML.format(asset="test_asset", layer="{a: b}")
        )

        with pytest.raises(ContractValidationError, match="Invalid layer"):
            load_contract_for_asset("test_asset", search_paths=[tmp_path])
...
class TestExpectationParamValidation:
    ...
    #
    # A mapping or list severity/when value must produce exactly one
    # error and never raise -- a bare `value not in {...}` membership
    # test calls hash(value) and crashes on these otherwise-legal YAML
    # values.

    @pytest.mark.parametrize("bad", [{"a": "b"}, ["a"]], ids=["mapping", "list"])
    def test_column_test_severity_unhashable_value_rejected_cleanly(self, bad: object) -> None:
        data = self._base()
        data["schema"]["columns"][0]["tests"] = [{"not_null": None, "severity": bad}]
        errors = validate_contract_schema(data)
        matches = [e for e in errors if "severity" in e]
        assert len(matches) == 1
        assert "must be one of" in matches[0]

    @pytest.mark.parametrize("bad", [{"a": "b"}, ["a"]], ids=["mapping", "list"])
    def test_column_test_when_unhashable_value_rejected_cleanly(self, bad: object) -> None:
        data = self._base()
        data["schema"]["columns"][0]["tests"] = [{"not_null": None, "when": bad}]
        errors = validate_contract_schema(data)
        matches = [e for e in errors if "when" in e]
        assert len(matches) == 1
        assert "must be one of" in matches[0]

    @pytest.mark.parametrize("bad", [{"a": "b"}, ["a"]], ids=["mapping", "list"])
    def test_expectation_severity_unhashable_value_rejected_cleanly(self, bad: object) -> None:
        data = self._base()
        data["expectations"] = [{"row_count": {"min": 1, "max": 10}, "severity": bad}]
        errors = validate_contract_schema(data)
        matches = [e for e in errors if "severity" in e]
        assert len(matches) == 1
        assert "must be one of" in matches[0]
