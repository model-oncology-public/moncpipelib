...
class TestFullRefreshTruncate:
    ...
    def test_batched_delete_method_keeps_delete_on_large_table(self) -> None:
        """The FK escape hatch: an explicit DELETE never truncates, never probes.

        A large, analyzed target that AUTO would truncate must still DELETE when
        the method is pinned -- this is what an FK-referenced target relies on
        so its streamed full refresh keeps working after #4 flips AUTO to
        TRUNCATE. Also proves the pinned method reaches the batched clear.
        """
        io_mgr = self.io_mgr_factory(
            db_schema="test_write",
            enable_row_lineage=False,
