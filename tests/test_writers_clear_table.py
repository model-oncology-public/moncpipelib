...
class TestClearTableAutoWithoutHint:
    ...
    def test_explicit_delete_with_unknown_hint_does_not_probe(self) -> None:
        """A pinned DELETE never consults the catalog, hint or no hint.

        This is the escape hatch FK-referenced targets rely on (#4): the probe
        and the truncate decision are both skipped.
        """
        cursor = _cursor(reltuples=5_000_000.0)
        _, method = clear_table(
            _config(FullRefreshMethod.DELETE), cursor, _TABLE, None, MagicMock()
        )
        assert method == "delete"
        assert not _probed_catalog(cursor)
