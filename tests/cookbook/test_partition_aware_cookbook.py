...
def test_partition_modes_summary() -> None:
    ...
    # +-----------------+----------------------------------------+------------------------+
    # | append          | No special handling; additive-only,     | No destructive-scope    |
    # |                 | never deletes, so no cross-partition    | guard. A declared       |
    # |                 | destruction to scope.                   | partition_column must   |
...
def test_partition_guard_rails() -> None:
    ...
    print()
    print("4. append + partitioned -- no destructive-scope guard (never deletes)")
    print()
    print("Applies in every write mode, append included (#480):")
    print("   a declared partition_column must be present in the DataFrame")
    print("   and free of NULLs -- NULL never matches the partition DELETE,")
    print("   so a NULL row is unreachable by every partition-scoped clear.")
    # --- cookbook:end ---
